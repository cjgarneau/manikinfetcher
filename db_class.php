...
class ANSURDataClass
	{
		...
		*/
		...
		public function allNames($dimCategory=0) {
			...
			if ($dimCategory>0 ){
									$sqlr = "SELECT abrevName, fullName, Units, category FROM NameTable WHERE category='".$dimCategory."'  ORDER BY fullName ASC";

			}else{
				
									$sqlr = "SELECT abrevName, fullName, Units, category, Availability FROM NameTable WHERE Availability = 1 ORDER BY category ASC, fullName ASC";
